...
def matmul_kernel(a,b,c,M:tl.constexpr,N:tl.constexpr,K:tl.constexpr,BM:tl.constexpr,BN:tl.constexpr,BK:tl.constexpr):
    ...
    for k0 in range(0,K,BK):
        av=tl.load(a+rm[:,None]*K+k0+rk[None,:], mask=(rm[:,None]<M)&(k0+rk[None,:]<K), other=0.0)
        bv=tl.load(b+(k0+rk[:,None])*N+rn[None,:], mask=(k0+rk[:,None]<K)&(rn[None,:]<N), other=0.0)
        acc += tl.dot(av,bv)
    tl.store(c+rm[:,None]*N+rn[None,:],acc,mask=(rm[:,None]<M)&(rn[None,:]<N))
def main():
    M=N=K=512; a=torch.randn((M,K),device="cuda"); b=torch.randn((K,N),device="cuda"); c=torch.empty((M,N),device="cuda")
    matmul_kernel[(triton.cdiv(M,16),triton.cdiv(N,16))](a,b,c,M,N,K,BM=16,BN=16,BK=32)
    err=(c-a@b).abs().max().item(); print(f"operator: triton_matmul\nGPU kernel: {'PASS' if err < 1e-2 else 'FAIL'}\nmax error: {err}\n{'PASS' if err < 1e-2 else 'FAIL'}")
