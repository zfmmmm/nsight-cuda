...
@triton.jit
def kernel(
    x, bias, residual, y, n: tl.constexpr, cols: tl.constexpr, BLOCK: tl.constexpr
):
...
def main():
    ...
    ref = (
        0.5
        * (x + b[torch.arange(n, device="cuda") % cols])
        * (
            1
            + torch.tanh(
                0.79788456
                * (
                    (x + b[torch.arange(n, device="cuda") % cols])
                    + 0.044715 * (x + b[torch.arange(n, device="cuda") % cols]) ** 3
                )
            )
        )
        + r
