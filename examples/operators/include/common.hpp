...
inline void fill_random(thrust::host_vector<float> &v, float lo = -1.0f, float hi = 1.0f,
                        unsigned seed = 123) {
...
inline void fill_random_int(thrust::host_vector<int> &v, int lo, int hi, unsigned seed = 123) {
    std::mt19937 gen(seed);
...
inline bool check_close(const thrust::host_vector<float> &a, const thrust::host_vector<float> &b,
                        float tol, double *max_err = nullptr) {
...
template <class F> float time_cuda_ms(F &&fn, int warmup = 5, int iters = 30) {
...
inline void print_result(const char *op, const char *shape, bool pass, double max_err, float ms,
                         double perf = 0.0, const char *perf_unit = "") {
