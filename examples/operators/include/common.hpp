...
inline void fill_random(
    thrust::host_vector<float>& v, float lo = -1.0f, float hi = 1.0f, unsigned seed = 123
) {
...
inline double max_abs_diff(
    const thrust::host_vector<float>& a, const thrust::host_vector<float>& b
) {
    double m = 0.0;
