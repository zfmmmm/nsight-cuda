...
int main()
{
    ...
    CUDNN_CHECK(cudnnSetPooling2dDescriptor(pd, CUDNN_POOLING_MAX, CUDNN_NOT_PROPAGATE_NAN, 2, 2, 0, 0, 2, 2));
    float a = 1, b = 0;
    auto  launch = [&] {
        CUDNN_CHECK(cudnnPoolingForward(
            h, pd, &a, xd, thrust::raw_pointer_cast(x.data()), &b, yd, thrust::raw_pointer_cast(y.data())));
    };
