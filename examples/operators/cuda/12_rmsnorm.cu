...
// 运行: ./12_rmsnorm
#include "common.hpp"
__inline__ __device__ float wrsum(float v) {
...
__global__ void rmsnorm_kernel(
    ...
    if ((t & 31) == 0) sm[t >> 5] = ss;
    __syncthreads();
    ss = (t < blockDim.x / 32) ? sm[t] : 0;
    if (t < 32) ss = wrsum(ss);
    if (t == 0) sm[0] = rsqrtf(ss / cols + eps);
    __syncthreads();
    float inv = sm[0];
    for (int c = t; c < cols; c += blockDim.x) y[r * cols + c] = w[c] * x[r * cols + c] * inv;
}
int main() {
    const int rows = 4096, cols = 1024, n = rows * cols;
    thrust::host_vector<float> h(n), w(cols), ref(n);
    fill_random(h);
    fill_random(w, 0.5, 1.5);
    for (int r = 0; r < rows; ++r) {
        ...
        float inv = 1 / std::sqrt(ss / cols + 1e-6f);
        for (int c = 0; c < cols; ++c) ref[r * cols + c] = w[c] * h[r * cols + c] * inv;
    }
    thrust::device_vector<float> d = h, dw = w, out(n);
    float ms = time_cuda_ms(
        [&] {
            rmsnorm_kernel<<<rows, 256, 32 * sizeof(float)>>>(
    ...
    print_result(
        "rmsnorm", "rows=4096,cols=1024", pass, err, ms, n * sizeof(float) * 2.0 / ms / 1e6, "GB/s"
    );
    return pass ? 0 : 1;
}
