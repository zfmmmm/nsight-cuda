...
//       m = m_new
//     O_i = acc / l
// 编译: nvcc -O3 -lineinfo -std=c++17 -I../include 23_flash_attention_forward.cu -o
// 23_flash_attention_forward 运行: ./23_flash_attention_forward
#include "common.hpp"

template <int D, int TILE>
__global__ void flash_attention_forward_kernel(
    const float* q, const float* k, const float* v, float* o, int seqlen
) {
    int row = blockIdx.x;
    int tid = threadIdx.x;
...
int main() {
    ...
    auto launch = [&] {
        flash_attention_forward_kernel<D, TILE><<<L, 128>>>(
    ...
    print_result(
        "flash_attention_forward_teaching",
        "single-head,L=128,D=64,TILE=32",
        pass,
        err,
        ms,
        flops,
