...
int main() {
  ...
  thrust::host_vector<float> h(n), ref(n); fill_random(h);
  for (int r = 0; r < rows; ++r) for (int c = 0; c < cols; ++c) ref[c * rows + r] = h[r * cols + c];
  thrust::device_vector<float> d = h, out1(n), out2(n);
  dim3 block(32, 32), grid((cols + 31) / 32, (rows + 31) / 32);
  float ms1 = time_cuda_ms([&]{ transpose_naive<<<grid, block>>>(raw(d), raw(out1), rows, cols); });
  float ms2 = time_cuda_ms([&]{ transpose_tiled<<<grid, block>>>(raw(d), raw(out2), rows, cols); });
  thrust::host_vector<float> got = out2;
  double err = 0; bool pass = check_close(ref, got, 1e-6f, &err);
  print_result("matrix_transpose_tiled", "2048x2048", pass, err, ms2, 2.0 * n * sizeof(float) / ms2 / 1e6, "GB/s");
