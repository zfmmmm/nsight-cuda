...
__global__ void hist_shared(const int* x, int* h, int n, int bins) {
    ...
    __syncthreads();
    for (int b = threadIdx.x; b < bins; b += blockDim.x) atomicAdd(&h[b], local[b]);
}
int main() {
    ...
    float ms1 = time_cuda_ms(
        [&] {
            zero(hg);
            hist_global<<<bl, th>>>(
                thrust::raw_pointer_cast(dx.data()), thrust::raw_pointer_cast(hg.data()), n, bins
            );
        },
        2,
        10
    ...
    float ms2 = time_cuda_ms(
        [&] {
            zero(hs);
            hist_shared<<<bl, th, bins * sizeof(int)>>>(
                thrust::raw_pointer_cast(dx.data()), thrust::raw_pointer_cast(hs.data()), n, bins
            );
        },
        2,
        10
    ...
    print_result(
        "histogram_shared_atomic",
        "n=4194304,bins=256",
        pass,
        maxerr,
        ms2,
        n * sizeof(int) / ms2 / 1e6,
        "GB/s"
    );
