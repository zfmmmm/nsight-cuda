...
#include "common.hpp"
__global__ void embedding_kernel(const float *table, const int *ids, float *out, int tokens,
                                 int dim) {
...
int main() {
    ...
    print_result("embedding_gather", "tokens=8192,dim=256", pass, err, ms,
                 tokens * dim * sizeof(float) * 2.0 / ms / 1e6, "GB/s");
    return pass ? 0 : 1;
}
