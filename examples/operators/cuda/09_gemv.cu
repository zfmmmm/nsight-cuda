...
int main() {
  const int M=4096,K=1024; thrust::host_vector<float> hA(M*K), hx(K), ref(M);
  fill_random(hA); fill_random(hx, -1, 1, 456);
  for(int m=0;m<M;++m){float s=0;for(int k=0;k<K;++k)s+=hA[m*K+k]*hx[k];ref[m]=s;}
  thrust::device_vector<float>A=hA,x=hx,y(M);
  float ms=time_cuda_ms([&]{gemv_kernel<<<M,256>>>(raw(A),raw(x),raw(y),M,K);});
  thrust::host_vector<float> got=y; double err=0; bool pass=check_close(ref,got,1e-3f,&err);
