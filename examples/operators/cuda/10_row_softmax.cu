...
__global__ void row_softmax_kernel(const float* x, float* y, int rows, int cols) {
    ...
    __syncthreads();
    m = (tid < blockDim.x / 32) ? sm[tid] : -3.402823466e38f;
    if (tid < 32) m = warp_reduce_max(m);
    if (tid == 0) sm[0] = m;
    __syncthreads();
    m = sm[0];
    float s = 0;
    for (int c = tid; c < cols; c += blockDim.x) {
        float e = expf(x[row * cols + c] - m);
        y[row * cols + c] = e;
        s += e;
    }
    s = warp_reduce_sum(s);
    ...
    __syncthreads();
    s = (tid < blockDim.x / 32) ? sm[tid] : 0;
    if (tid < 32) s = warp_reduce_sum(s);
    if (tid == 0) sm[0] = s;
    __syncthreads();
    s = sm[0];
...
int main() {
    ...
    for (int r = 0; r < rows; ++r) {
        ...
        for (int c = 0; c < cols; ++c) ref[r * cols + c] /= s;
    }
    thrust::device_vector<float> d = h, out(n);
    float ms = time_cuda_ms(
        [&] {
            row_softmax_kernel<<<rows, 256, 32 * sizeof(float)>>>(
                thrust::raw_pointer_cast(d.data()), thrust::raw_pointer_cast(out.data()), rows, cols
            );
        },
        3,
        20
