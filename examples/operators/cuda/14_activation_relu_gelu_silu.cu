...
// 编译: nvcc -O3 -lineinfo -std=c++17 -I../include 14_activation_relu_gelu_silu.cu -o
// 14_activation_relu_gelu_silu 运行: ./14_activation_relu_gelu_silu
#include "common.hpp"
...
int main() {
    const int n = 1 << 24;
    thrust::host_vector<float> h(n), r(n), g(n), s(n);
    fill_random(h, -5, 5);
    for (int i = 0; i < n; ++i) {
        float v = h[i];
        r[i] = std::max(v, 0.0f);
        g[i] = 0.5f * v * (1 + std::tanh(0.79788456f * (v + 0.044715f * v * v * v)));
        s[i] = v / (1 + std::exp(-v));
    }
