...
__global__ void reduce_max_kernel(const float *x, float *partial, int n) {
  ...
  for (int i = blockIdx.x * blockDim.x + threadIdx.x; i < n; i += blockDim.x * gridDim.x) m = fmaxf(m, x[i]);
  m = warp_max(m);
...
int main() {
  ...
  thrust::host_vector<float> h(n); fill_random(h, -10, 10);
  ...
  double err = std::abs(ref - got[0]); bool pass = err < 1e-6;
