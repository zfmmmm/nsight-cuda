...
__device__ Pair better(Pair a, Pair b) {
    return (b.v > a.v || (b.v == a.v && b.idx < a.idx)) ? b : a;
}
...
int main() {
    ...
    fill_random(h, -5, 5);
    h[n / 3] = 99.0f;
    int ref_idx = std::max_element(h.begin(), h.end()) - h.begin();
    float ref_v = h[ref_idx];
    thrust::device_vector<float> d = h;
    thrust::device_vector<Pair> partial(blocks), out(1);
