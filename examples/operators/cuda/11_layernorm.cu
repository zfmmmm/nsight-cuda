...
__global__ void layernorm_kernel(const float *x, const float *g, const float *b, float *y, int rows,
    ...
    __syncthreads();
    s = (t < blockDim.x / 32) ? sm[t] : 0;
    ss = (t < blockDim.x / 32) ? sm[32 + t] : 0;
    if (t < 32) {
        s = wsum(s);
        ss = wsum(ss);
    }
    if (t == 0) {
        sm[0] = s / cols;
        sm[1] = ss / cols - sm[0] * sm[0];
    }
    __syncthreads();
    float mean = sm[0], inv = rsqrtf(sm[1] + eps);
    for (int c = t; c < cols; c += blockDim.x)
        y[r * cols + c] = g[c] * (x[r * cols + c] - mean) * inv + b[c];
}
int main() {
    const int rows = 2048, cols = 1024, n = rows * cols;
    thrust::host_vector<float> h(n), g(cols), b(cols), ref(n);
    fill_random(h);
    fill_random(g, 0.5, 1.5);
    fill_random(b, -0.1, 0.1);
    for (int r = 0; r < rows; ++r) {
        ...
        float m = s / cols, var = ss / cols - m * m, inv = 1 / std::sqrt(var + 1e-5f);
        for (int c = 0; c < cols; ++c)
            ref[r * cols + c] = g[c] * (h[r * cols + c] - m) * inv + b[c];
    }
